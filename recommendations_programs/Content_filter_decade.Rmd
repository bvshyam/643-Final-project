...
#colnames(predictions) <- c("user","isbn","title","DecadeYearPublished", "AvgRating")
...
fetchforuser <- c( "900")
predictions <- getDecadeBasedRecm(fetchforuser, 10)
unique(predictions$year) # Which are the decades predicted for.
head(predictions,10)

# Testing for User 901

fetchforuser <- c( "901")
predictions <- getDecadeBasedRecm(fetchforuser, 2)
unique(predictions$year) # Which are the decades predicted for.
head(predictions, 2)
