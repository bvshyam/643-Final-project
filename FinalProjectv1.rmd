---
title: "Recommender System : Book Crossing"
...
length(unique(combinedData$isbn))
length(unique(combinedData$user))
dim(combinedData)
...
length(unique(dfbookratingsvalid$isbn))
length(unique(dfbookratingsvalid$user))
...
# Taking a transpose to have book (rows) by user (columns), This is wide format but books as rows and users and columns
...
#datatable(head(dfbookratingswide, 10))
#datatable(head(dfusers, 10))
#datatable(head(dfbooks, 10))

# TO COMMENT LATER
#View(head(dfbookratingsnew, 50000))
#View(head(dfusers, 10))
#View(head(dfbooks, 10))
#View(dfbooks)
#View(dfusers)

# We now have dfbookratingsvalid(Long format) , 
# dfbookratingswide (wide format user~books)
# dfbookratingswidet (wide format books~dfbooks2$yearpub)
#dfusers, dfbooks

# We keep and refer only the books that are rated, hence putting that i a new bookds data set dfbooks2
